...
if [ "$NOOPENMPGPU" = 1 ]; then
	VR_CMAKE_OPTIONS+=" -DVR_OPENMPGPU=OFF "
fi
if [ "$OPENACC" = 1 ]; then
	VR_CMAKE_OPTIONS+=" -DVR_OPENACC=ON "
fi
if [ "$CLANG10" = 1 ]; then
	VR_CMAKE_OPTIONS+=" -DCMAKE_CXX_COMPILER=clang++-10 -DCMAKE_C_COMPILER=clang-10 -DCMAKE_CXX_FLAGS=-std=c++11 "
fi
if [ "$CLANG9" = 1 ]; then
	VR_CMAKE_OPTIONS+=" -DCMAKE_CXX_COMPILER=clang++-9 -DCMAKE_C_COMPILER=clang-9 -DCMAKE_CXX_FLAGS=-std=c++11 "
fi
