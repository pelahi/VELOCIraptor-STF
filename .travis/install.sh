...
if [ "$CLANG10" = 1 ]; then
	VR_CMAKE_OPTIONS+=" -DCMAKE_CXX_COMPILER=clang++-10 -DCMAKE_C_COMPILER=clang-10 -DCMAKE_CXX_FLAGS=-lstdc++"
fi
if [ "$CLANG9" = 1 ]; then
	VR_CMAKE_OPTIONS+=" -DCMAKE_CXX_COMPILER=clang++-9 -DCMAKE_C_COMPILER=clang-9 -DCMAKE_CXX_FLAGS=-lstdc++"
fi
