...
#ifdef HIGHRES
                    else if (k==HDFDM1TYPE) {
                        if (opt.iUseHighResOnlyFOF == 0) Part[count].SetType(DARKTYPE);
                        else Part[count].SetType(DARKTYPELOWRES);
                    }
                    else if (k==HDFDM2TYPE) {
                        if (opt.iUseHighResOnlyFOF == 0) Part[count].SetType(DARKTYPE);
                        else Part[count].SetType(DARKTYPELOWRES);
                    }
#endif
                  ...
                  for(n=0;n<hdf_header_info[i].npart[k];n+=nchunk)
                  {
                    ...
                    for (int nn=0;nn<nchunk;nn++) {
                        for (auto kk=0;kk<3;kk++) {
                              if (doublebuff[nn*3+kk]<0) doublebuff[nn*3+kk]+=opt.p;
                              else if (doublebuff[nn*3+kk]>opt.p) doublebuff[nn*3+kk]-=opt.p;
                        }
