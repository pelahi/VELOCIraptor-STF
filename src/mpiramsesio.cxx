...
void MPIDomainDecompositionWithTree(Options &opt){
	...
	vector<Particle> Part_mpi;
	Part_mpi.resize(nbodies);
	...
	//-----
        ...
        RAMSESFLOAT *xtempchunk, *mtempchunk, *agetempchunk, *xtempall;
	int *famtempall, *dummy_family;
	char *dummy_family2;
	Double_t mscale,lscale,lvscale,rhoscale,scale_l,scale_d,scale_t;
	...
	//-----
	  ...
	  Finfo>>stringbuf>>stringbuf>>scale_l;
	  Finfo>>stringbuf>>stringbuf>>scale_d;
	  Finfo>>stringbuf>>stringbuf>>scale_t;

          Finfo.close();
          dmp_mass = 1.0 / (opt.Neff*opt.Neff*opt.Neff) * (OmegaM - OmegaB) / OmegaM;


	  lscale	= scale_l/3.086e21;
	  lvscale	= scale_l/scale_t*1e-5;
	  mscale	= scale_d*(scale_l * scale_l * scale_l)/1.988e+33;
        }
...
//                            if (fabs(mtemp-dmp_mass)/dmp_mass<1e-5)
			    ...
			    if(fabs(mtemp-dmp_mass)/dmp_mass<1e-5 && dummy_family[nn] == 1)
                            {
                                typeval = DARKTYPE;

				if(opt.partsearchtype==PSTALL||opt.partsearchtype==PSTDARK){
				  Part_mpi[count_mpi]=Particle(mtemp*mscale,
					xtemp[0]*lscale,xtemp[1]*lscale,xtemp[2]*lscale,
					0.,0.,0.,
					ndark+nstar,typeval);
				  xtempall[count_mpi] = xtemp[0];
				  xtempall[count_mpi + nbodies] = xtemp[1];
				  xtempall[count_mpi + nbodies*2] = xtemp[2];
				  famtempall[count_mpi] = typeval;
				  count_mpi++;
				}
                                ndark++;
                            }
                            else if(dummy_family[nn] == 2)
                            {
                                typeval = STARTYPE;
	    ...
	    if(opt.iBaryonSearch>0){
	    	cout<<"%123123-----"<<endl;
	    	cout<<"%123123	MPIDomainDecompositionWithTree: Read AMR part should be added"<<endl;
	    	cout<<"%123123-----"<<endl;
	    	exit(9);
	    }
	    ...
	    ////-----
	    rdist	= (opt.ellxscale*opt.ellxscale)*(opt.ellphys*opt.ellphys)*(opt.ellhalophysfac*opt.ellhalophysfac);
	    impi_bsize	= nbodies/NProcs;
	    tree	= new KDTree(rdist, Part_mpi.data(), nbodies, impi_bsize, tree->TPHYS, tree->KEPAN, 100);
	    Node *root = tree->GetRoot();

	    // Adjust Nodes
	    Int_t impi_nleaf = tree->GetNumLeafNodes();
	    Int_t *impi_leafID;
	    Int_t impi_index;
	    Int_t impi_nodeid;
	    while(impi_nleaf != NProcs){
	        impi_leafID = new Int_t[impi_nleaf];
		for(Int_t i2=0; i2<impi_nleaf; i2++) impi_leafID[i2] = -1;
	    ...
	    // Set Boundary
	    impi_index = 0;
	    MPIDomainDecompositionWithTree_SetBnd(root, impi_index, lscale);

	    delete tree;

	    ////-----
	    //// Determine Processor to particles
	    ////-----
	    for(Int_t i2=0; i2<nbodies; i2++){
		ibuf = MPIGetParticlesProcessor(opt, xtempall[i2], xtempall[i2+nbodies], xtempall[i2+nbodies*2]);
			...
			if(opt.partsearchtype == PSTDARK)
			{
				if(famtempall[i2] == DARKTYPE)
				{
					...
					if(opt.iBaryonSearch){
						Nbaryonbuf[ibuf]++;
						cout<<"%123123-----"<<endl;
						cout<<"%123123 Mpiramsesio.cxx: should be updated here"<<endl;
						cout<<"%123123-----"<<endl;
						exit(9);
					}
				}
	    ...
	    }// Endfor for the particle loop
	}// Endif for reading task
	...
	// DEALLOCATE
	//-----
	delete[] xtempall, famtempall;
	delete[] header;
	delete[] Fpart, Fpartmass, Fpartage, Fpartfam;
	delete[] Famr, Fhydro;
	delete[] ireadtask, readtaskID, ireadfile;
	delete[] Nbuf, Nbaryonbuf;
...
void MPINumInDomainRAMSES(Options &opt)
{
	...
	if(opt.impiusetree){
		MPIDomainDecompositionWithTree(opt);
		return;
	}
